...
{
    //
    SerialUSB.print(F("  FAULT DATA = "));
    SerialUSB.println(fault_byte, BIN);

    if (fault_byte & SENSOR_HARD_FAILURE)
        SerialUSB.println(F("  - SENSOR HARD FALURE"));
    if (fault_byte & ADC_HARD_FAILURE)
        SerialUSB.println(F("  - ADC_HARD_FAILURE"));
    if (fault_byte & CJ_HARD_FAILURE)
        SerialUSB.println(F("  - CJ_HARD_FAILURE"));
    if (fault_byte & CJ_SOFT_FAILURE)
        SerialUSB.println(F("  - CJ_SOFT_FAILURE"));
    if (fault_byte & SENSOR_ABOVE)
        SerialUSB.println(F("  - SENSOR_ABOVE"));
    if (fault_byte & SENSOR_BELOW)
        SerialUSB.println(F("  - SENSOR_BELOW"));
    if (fault_byte & ADC_RANGE_ERROR)
        SerialUSB.println(F("  - ADC_RANGE_ERROR"));
    if (!(fault_byte & VALID))
        SerialUSB.println(F("INVALID READING !!!!!!"));
...
uint32_t transfer_four_bytes(uint8_t chip_select, uint8_t ram_read_or_write, uint16_t start_address, uint32_t input_data)
{
    ...
    output_data = (uint32_t)rx[3] << 24 | (uint32_t)rx[2] << 16 | (uint32_t)rx[1] << 8 | (uint32_t)rx[0];
...
void spi_transfer_block(uint8_t chip_select, uint8_t* tx, uint8_t* rx, uint8_t length)
{
    ...
    for (i = (length - 1); i >= 0; i--)
        ...
        case 0:
            ...
            break;
        case 1:
            rx[i] = SPI1.transfer(tx[i]);
            break;
        case 2:
            rx[i] = SPI2.transfer(tx[i]);
            break;
        default:
